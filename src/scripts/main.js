...
const eachVehicle =
    salesByWeek.forEach(sale => {
        ...
        displayContainer.innerHTML += `<h1>${sale.sales_agent.first_name} ${sale.sales_agent.last_name}</h1>`


        for (const entry of Object.entries(sale.vehicle)) {
            displayContainer.innerHTML += `<div>${entry[0]}: ${entry[1]}</div>`
        }

        displayContainer.innerHTML += `<h2>${sale.gross_profit}</h2>`
